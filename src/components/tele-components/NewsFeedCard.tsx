                ...
                {visible.map((a, i) => (
                    <div key={i} className="flex items-start gap-2 p-1.5 rounded-sm border-b" style={{ borderColor: `${getColor(4)}` }}>
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="font-data text-body font-bold leading-tight" style={{ color: `${getColor(90)}` }}>{a.headline}</div>
                            <div className="flex items-center gap-2 mt-0.5">
                                {a.source && <span className="font-data text-body uppercase" style={{ color: `${getColor(60)}` }}>{a.source}</span>}
