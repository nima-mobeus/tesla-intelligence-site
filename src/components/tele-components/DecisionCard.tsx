...
export default function DecisionCard({ data, accentColor, onAction }: TeleComponentProps) {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            {title && (
                <h3 className="font-data text-body uppercase tracking-[0.12em] mb-1" style={{ color: `${getColor(90)}` }}>
                    {title}
                </h3>
            )}

            {/* Subject */}
            <div className="font-data text-body font-bold leading-tight mb-1" style={{ color: C }}>
                {subject}
            </div>
            ...
            <div className="flex items-center gap-2 mb-2">
                <span
                    className="font-data text-body uppercase tracking-wider px-1.5 py-0.5 rounded font-bold"
                    style={{ backgroundColor: style.bg, color: style.color }}
                >
                    {urgency}
                </span>
                {deadline && (
                    <span className="font-data text-body md:text-body" style={{ color: `${getColor(70)}` }}>
                ...
                <div className="flex flex-col gap-1 mb-2 flex-1 min-h-0 overflow-hidden">
                    {visible.map((opt, i) => (
                        <div key={i} className="flex items-center gap-1.5">
                            ...
                            <span className="font-data text-body leading-tight" style={{ color: C }}>
                                {opt.label}
                            </span>
                            {opt.recommender && (
                                <span className="font-voice text-body italic" style={{ color: `${getColor(60)}` }}>
                                    — {opt.recommender}
                                </span>
                            )}
                        </div>
            ...
            {/* Consequence */}
            {consequence && (
                <div className="font-voice text-body leading-tight mt-auto pt-1 border-t" style={{ color: `${getColor(70)}`, borderColor: `${getColor(8)}` }}>
            ...
            {/* Footer: owner + source */}
            {(owner || source) && (
                <div className="font-data text-body mt-1 flex gap-2" style={{ color: `${getColor(40)}` }}>
