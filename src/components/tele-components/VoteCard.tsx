...
export default function VoteCard({ data, accentColor, onAction }: TeleComponentProps) {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            {/* Header */}
            {title && (
                <h3 className="font-data text-body uppercase tracking-[0.12em] mb-0.5 text-card-primary">
                    {title}
                </h3>
            )}
            ...
            {/* Description */}
            {description && (
                <p className="font-voice text-body leading-tight mb-2 line-clamp-2 text-card-secondary">
                    {description}
                </p>
            )}
            ...
            <div className="flex items-center gap-2 mb-2">
                <div className="flex-1 h-2 rounded-full overflow-hidden bg-card-surface">
                ...
                <div className="flex flex-col gap-0.5 flex-1 min-h-0 overflow-hidden">
                    {visible.map((pos, i) => (
                        <div key={i} className="flex items-center gap-1.5">
                            ...
                            {pos.condition && (
                                <span className="font-voice text-body italic truncate text-card-faint">
                                    — {pos.condition}
                                </span>
                            )}
                        </div>
            ...
            {prepActions.length > 0 && (
                <div className="mt-1 pt-1 border-t" style={{ borderColor: `${getColor(8)}` }}>
                    {prepActions.slice(0, 2).map((action, i) => (
                        <div key={i} className="font-voice text-body leading-tight text-card-muted">
                            → {action}
                        </div>
                    ))}
                </div>
