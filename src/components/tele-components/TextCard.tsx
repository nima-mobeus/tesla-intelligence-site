...
export default function TextCard({ data, accentColor, onAction }: TeleComponentProps) {
    const { title, body, bullets } = data as TextCardData;
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex-1 flex flex-col justify-center min-h-0 overflow-hidden">
                {body && (
                    <p className="font-voice text-body  leading-relaxed line-clamp-4 text-card-secondary">
                        {body}
                    </p>
                )}
                    ...
                    <ul className={`space-y-1.5 ${body ? 'mt-3' : ''}`}>
                        {visibleBullets.map((b, i) => (
                            <li key={i} className="flex items-start gap-2">
                                ...
                                <span className="font-voice text-body leading-snug text-card-primary">
                                    {b}
                                </span>
                            </li>
                        ))}
