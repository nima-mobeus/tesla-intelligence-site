                    ...
                    return (
                        <div key={i} className="flex items-center gap-2 p-1.5 rounded-sm border-l-2" style={{ backgroundColor: `${clr}08`, borderColor: clr }}>
                            <div className="flex-1 min-w-0">
                                <div className="font-data text-body font-bold truncate" style={{ color: `${getColor(90)}` }}>{r.route}</div>
                                {r.detail && <div className="font-voice text-body leading-tight mt-0.5" style={{ color: `${getColor(60)}` }}>{r.detail}</div>}
                            </div>
                            <div className="flex items-center gap-2 shrink-0">
                                {r.eta && <span className="font-data text-body font-bold" style={{ color: `${getColor(90)}` }}>{r.eta}</span>}
                                {r.delay && <span className="font-data text-body font-bold px-1 py-0.5 rounded-sm" style={{ backgroundColor: `${clr}20`, color: clr }}>{r.delay}</span>}
