                ...
                {visible.map((item, i) => {
                    ...
                    return (
                        <div key={i} className="flex items-center gap-2">
                            <span className="font-data text-body font-bold w-4 text-right shrink-0" style={{ color: `${getColor(70)}` }}>#{i + 1}</span>
                            <span className="font-data text-body w-20 truncate font-bold" style={{ color: `${getColor(70)}` }}>{item.label}</span>
                            ...
                            <span className="font-data text-body font-bold min-w-[40px] text-right shrink-0" style={{ color: `${getColor(90)}` }}>
                                {item.displayValue || (item.value ?? 0).toLocaleString()}{unit ? ` ${unit}` : ''}
                            </span>
                            {item.change && (
                                <span className="font-data text-body font-bold min-w-[32px] text-right" style={{ color: item.change.startsWith('+') ? '#22c55e' : '#ff4040' }}>
                                    {item.change}
                                </span>
                            )}
                        </div>
