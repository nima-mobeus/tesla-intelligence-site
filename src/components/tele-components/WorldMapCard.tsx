                    ...
                    return (
                        <div key={i} className="flex items-center gap-2 p-1.5 rounded-sm" style={{ backgroundColor: `${getColor(2)}`, borderLeft: `3px solid ${clr}` }}>
                            <div className="min-w-0">
                                <div className="font-data text-body font-bold uppercase truncate" style={{ color: `${getColor(90)}` }}>{r.name}</div>
                                <div className="font-data text-body font-bold" style={{ color: clr }}>{r.value}</div>
                                {r.subtext && <div className="font-data text-body" style={{ color: `${getColor(70)}` }}>{r.subtext}</div>}
                            </div>
                        </div>
                    );
                })}
