    ...
    return (
        <div className="flex flex-col h-full justify-start gap-1.5">
            <div className="flex items-center gap-2">
                ...
                <h3 className="font-data text-body font-bold tracking-wide" style={{ color: `${getColor(90)}` }}>{destination}</h3>
            </div>
            {dates && (
                <span className="flex items-center gap-1 font-data text-body tracking-wider" style={{ color: `${getColor(38)}` }}>
                    <CalendarDays size={10} /> {dates}
                </span>
            )}
            {purpose && (
                <p className="font-voice text-body leading-snug" style={{ color: `${getColor(60)}` }}>{purpose}</p>
            )}
            {flight && (
                <div className="px-2 py-1.5 rounded" style={{ backgroundColor: `${getColor(2)}`, border: `1px solid ${getColor(6)}` }}>
                    <div className="flex items-center gap-1 font-data text-body tracking-wide" style={{ color: `${getColor(60)}` }}>
                        <Plane size={10} /> {flight}
                    </div>
                    {flightTime && (
                        <div className="font-data text-body tracking-wider" style={{ color: `${getColor(31)}` }}>
            ...
            {companions && (
                <div className="flex items-center gap-1 font-data text-body tracking-wider" style={{ color: `${getColor(25)}` }}>
                    <Users size={10} /> {companions}
                </div>
            )}
            {keyMeeting && (
                <div className="flex items-center gap-1 font-voice text-body italic mt-auto" style={{ color: `${getColor(31)}` }}>
