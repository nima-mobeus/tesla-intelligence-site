                ...
                {[left, right].map((side, si) => (
                    <div key={si} className="flex flex-col items-center gap-2 p-2 rounded-sm" style={{ backgroundColor: `${getColor(2)}` }}>
                        <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ backgroundColor: `${getColor(8)}` }}>
                            <span className="font-data text-body font-bold" style={{ color: `${getColor(90)}` }}>{side.name?.[0]}</span>
                        </div>
                        <div className="text-center">
                            <div className="font-data text-body font-bold" style={{ color: `${getColor(90)}` }}>{side.name}</div>
                            {side.title && <div className="font-data text-body uppercase" style={{ color: `${getColor(85)}` }}>{side.title}</div>}
                        </div>
                        <div className="w-full space-y-1">
                            {side.metrics.map((m, mi) => (
                                <div key={mi} className="flex justify-between">
                                    <span className="font-data text-body" style={{ color: `${getColor(85)}` }}>{m.label}</span>
                                    <span className="font-data text-body font-bold" style={{ color: `${getColor(90)}` }}>{m.value}</span>
                                </div>
                            ))}
                        </div>
                    </div>
