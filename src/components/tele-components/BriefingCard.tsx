...
export default function BriefingCard({ data, accentColor, onAction }: TeleComponentProps) {
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden justify-center gap-2 px-2">
            {/* Title */}
            <h3 className="font-hero text-title leading-snug line-clamp-2" style={{ color: C }}>
                {title}
            </h3>

            {/* Subtitle / To line */}
            {subtitle && (
                <div className="font-data text-body uppercase tracking-[0.15em]" style={{ color: `${getColor(70)}` }}>
                    {subtitle}
                </div>
            )}

            {/* Body text — full paragraph style */}
            <p className="font-voice text-body leading-relaxed whitespace-pre-line line-clamp-4" style={{ color: `${getColor(88)}` }}>
                {body}
            </p>
            ...
            {cta && (
                <button
                    className="self-start font-data text-body uppercase tracking-wider px-4 py-2 rounded-sm transition-colors mt-1"
