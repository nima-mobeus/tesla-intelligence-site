    ...
    <div className="w-full overflow-hidden rounded-lg border" style={{ borderColor: accentColor }}>
      ...
      <div className="space-y-3 p-4">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-title font-semibold">{data.name as string}</h3>
            {data.badge && (
              <span className="mt-1 inline-block rounded-full px-2.5 py-0.5 text-caption font-medium text-white" style={{ backgroundColor: accentColor }}>
          ...
          {data.price && (
            <div className="text-right">
              <div className="text-title font-semibold" style={{ color: accentColor }}>{data.price as string}</div>
              {data.currency && <div className="text-caption text-muted-foreground">{data.currency as string}</div>}
            </div>
          )}
        </div>

        {data.description && <p className="text-body text-muted-foreground">{data.description as string}</p>}
        ...
        {tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {tags.map((tag) => (
              <span key={tag} className="rounded-full border px-2 py-0.5 text-caption">{tag}</span>
            ))}
          </div>
        )}
