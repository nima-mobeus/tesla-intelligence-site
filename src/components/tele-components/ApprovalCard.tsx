    ...
    return (
        <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-2">
                {title && (
                    <h3 className="font-data text-body uppercase tracking-[0.12em] text-card-primary">
                        {title}
                    </h3>
                )}
                {pending > 0 && (
                        ...
                        <div key={i} className="flex items-start gap-2 py-1 border-b" style={{ borderColor: `${getColor(6)}` }}>
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                    {item.from && (
                                        <span className="font-voice text-body text-card-secondary">
                                            From: {item.from}{item.fromTitle ? ` (${item.fromTitle})` : ''}
                                        </span>
                                    )}
                                    {item.deadline && (
                                        <span className="font-data text-body text-card-muted">
                                            ⏰ {item.deadline}
                                        </span>
                                    )}
                                </div>
