        ...
        <div className="flex flex-col h-full overflow-hidden">
            <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: `${getColor(8)}` }}>
                    <span className="font-data text-micro font-bold" style={{ color: `${getColor(90)}` }}>{teamName?.[0]}</span>
                </div>
                <div>
                    <div className="font-data text-body font-bold" style={{ color: `${getColor(90)}` }}>{teamName}</div>
                    {teamLead && <div className="font-data text-body uppercase tracking-wider" style={{ color: `${getColor(70)}` }}>Lead: {teamLead}</div>}
                </div>
            </div>
                ...
                {kpis.map((k, i) => (
                    <div key={i} className="flex items-center justify-between py-1 border-b" style={{ borderColor: `${getColor(4)}` }}>
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-data text-body md:text-body" style={{ color: `${getColor(88)}` }}>{k.label}</span>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="font-data text-body font-bold" style={{ color: `${getColor(90)}` }}>{k.value}</span>
                            {k.change && <span className="font-data text-body font-bold" style={{ color: k.change.startsWith('+') ? '#22c55e' : '#ff4040' }}>{k.change}</span>}
                        </div>
                    </div>
                ))}
            </div>
