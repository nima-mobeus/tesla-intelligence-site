            ...
            <div className="flex-1 flex flex-col justify-center min-h-0 overflow-hidden min-h-0">
                ...
                {labels && labels.length > 0 && (
                    <div className="flex justify-between mt-1">
                        {labels.map((l, i) => (
                            <span key={i} className="font-data text-body uppercase text-card-primary">
                                {l}
                            </span>
                        ))}
                    </div>
                ...
                {/* Min/Max annotation */}
                <div className="flex justify-between mt-1">
                    <span className="font-data text-body font-medium text-card-primary">
                        {(mn ?? 0).toLocaleString()}{unit ? ` ${unit}` : ''}
                    </span>
                    <span className="font-data text-body font-bold text-card-primary">
