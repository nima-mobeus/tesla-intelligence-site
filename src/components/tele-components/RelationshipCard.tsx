  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-12 h-12 rounded-full flex items-center justify-center shrink-0 bg-card-surface">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-data text-body font-bold leading-tight truncate text-card-primary">{name}</div>
          {role && <div className="font-data text-body uppercase tracking-wider truncate text-card-muted">{role}</div>}
        </div>
        <div className="flex items-center gap-1 shrink-0">
          ...
          <span className="font-data text-body uppercase tracking-wider font-bold" style={{ color: s.text }}>{sentiment.replace('-', ' ')}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 mb-2">
        <div className="flex items-center gap-1">
          ...
          <span className="font-data text-body uppercase tracking-wider text-card-muted">{trajectory}</span>
        </div>
        {lastContact && (
          <div className="font-data text-body text-card-muted">
        ...
        {daysSince !== undefined && (
          <span className="font-data text-body font-bold px-1.5 py-0.5 rounded"
            style={{
      ...
      {commitments.length > 0 && (
        <div className="flex flex-col gap-0.5 mb-2 flex-1 min-h-0 overflow-hidden">
          <span className="font-data text-body uppercase tracking-wider text-card-faint">Open commitments</span>
          {commitments.slice(0, 3).map((c, i) => (
            <div key={i} className="flex items-start gap-1.5">
              ...
              <span className="font-voice text-body leading-tight" style={{ color: getColor(75) }}>{c}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {actionNeeded && (
        <div className="font-data text-body px-2 py-1.5 rounded-sm mt-auto border-l-2"
      ...
      {riskLevel && (
        <div className="flex items-center gap-1 mt-1 pt-1 border-t" style={{ borderColor: getColor(8) }}>
          <span className="font-data text-body uppercase tracking-wider text-card-faint">Risk:</span>
