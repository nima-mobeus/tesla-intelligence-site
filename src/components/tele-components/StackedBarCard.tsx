        ...
        {groups.map((g, gi) => {
          ...
          return (
            <div key={gi} className="flex items-center gap-1.5">
              <span className="font-data text-body w-14 text-left truncate font-bold" style={{ color: getColor(70) }}>{g.label}</span>
              ...
              <span className="font-data text-body font-bold min-w-[36px] text-right" style={{ color: getColor(90) }}>
      ...
      <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1">
        {allLabels.map((l, i) => (
          <div key={i} className="flex items-center gap-1">
            ...
            <span className="font-data text-body" style={{ color: getColor(70) }}>{l}</span>
          </div>
        ))}
      </div>
    </div>
