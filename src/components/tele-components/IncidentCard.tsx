...
export default function IncidentCard({ data, accentColor, onAction }: TeleComponentProps) {
    ...
    return (
        <div className="flex flex-col h-full gap-2 rounded-sm p-2 border-l-2" style={{ backgroundColor: sev.bg, borderColor: sev.border }}>
            ...
            <h3 className="font-data text-body font-bold text-card-primary">{title}</h3>
            {summary && <p className="font-voice text-body leading-relaxed text-card-primary">{summary}</p>}
                ...
                <div className="space-y-1 ml-1">
                    {visibleTimeline.map((t, i) => (
                        <div key={i} className="flex items-start gap-2">
                            ...
                            <span className="font-voice text-body leading-tight text-card-primary">{t.description}</span>
                        </div>
                    ))}
                </div>
            )}
