...
export default function EmailListCard({ data, accentColor, onAction }: TeleComponentProps) {
    ...
    return (
        <div className="flex flex-col h-full justify-start gap-2">
            {title && (
                <h3 className="flex items-center gap-1.5 font-data text-body tracking-[0.15em] uppercase font-semibold text-card-muted">
                    <Mail size={12} /> {title}
                </h3>
            )}
                    ...
                    <div key={i} className="flex items-start gap-2 py-1.5 border-b" style={{ borderColor: `${getColor(4)}` }}>
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-baseline gap-1.5">
                                <span className={`font-data text-body tracking-wide truncate ${email.unread ? 'font-bold' : ''} text-card-primary`}>{email.from}</span>
                                {email.fromTitle && (
                            ...
                            <div className="font-voice text-body truncate leading-snug" style={{ color: `${getColor(44)}` }}>{email.subject}</div>
                        </div>
                        <div className="shrink-0 flex flex-col items-end gap-0.5">
                            {email.time && (
                                <span className="font-data text-body tracking-wider" style={{ color: `${getColor(21)}` }}>{email.time}</span>
                            )}
                            {email.replyNeeded && (
                                <span className="flex items-center gap-0.5 font-data text-body tracking-wider uppercase px-1 py-0.5 rounded-full" style={{ color: '#ef4444', border: '1px solid #ef444425' }}>
