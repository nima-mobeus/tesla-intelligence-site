  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-start gap-2 mb-1">
        ...
        {date && <span className="font-data text-body shrink-0 tracking-wider" style={{ color: getColor(50) }}>{date}</span>}
      </div>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {speed && (
          <span className="font-data text-body px-1.5 py-0.5 rounded"
      ...
      <div className="flex gap-3 mb-2 flex-1 min-h-0 overflow-hidden">
        {dataAvailable.length > 0 && (
          <div className="flex-1 min-w-0">
            <div className="font-data text-body uppercase tracking-wider mb-0.5" style={{ color: '#22c55e' }}>Had ✓</div>
            {dataAvailable.slice(0, 3).map((d, i) => (
              <div key={i} className="font-voice text-body leading-tight truncate" style={{ color: getColor(65) }}>{d}</div>
            ))}
          </div>
        )}
        {dataMissing.length > 0 && (
          <div className="flex-1 min-w-0">
            <div className="font-data text-body uppercase tracking-wider mb-0.5" style={{ color: '#b45309' }}>Missing ⚠</div>
      ...
      {(expectedOutcome || actualOutcome) && (
        <div className="pt-1 border-t space-y-0.5" style={{ borderColor: getColor(8) }}>
          {expectedOutcome && (
            <div className="font-voice text-body leading-tight" style={{ color: getColor(60) }}>
              <span className="font-data text-body uppercase tracking-wider" style={{ color: getColor(40) }}>Expected: </span>{expectedOutcome}
            </div>
          )}
          {actualOutcome && (
            <div className="font-voice text-body leading-tight" style={{ color: acc.text }}>
              <span className="font-data text-body uppercase tracking-wider" style={{ color: getColor(40) }}>Actual: </span>{actualOutcome}
            </div>
          )}
        </div>
      )}
