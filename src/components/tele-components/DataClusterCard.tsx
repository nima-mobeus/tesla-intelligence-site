            ...
            <div className="flex-1 grid grid-cols-2 md:grid-cols-3 gap-2 content-center">
                {metrics.map((m, i) => (
                    <div key={i} className="text-center p-1.5 rounded-sm" style={{ backgroundColor: `${getColor(2)}` }}>
                        <div className="font-data text-body uppercase tracking-wider" style={{ color: `${getColor(85)}` }}>{m.label}</div>
                        <div className="font-hero text-body md:text-title leading-tight" style={{ color: `${getColor(90)}` }}>{m.value}</div>
                        {m.trend && m.change && (
                            <div className="font-data text-body font-bold" style={{ color: TREND_CLR[m.trend] }}>
